...
let ``prepareTextMessage returns Some``() =
  ...
  | Some textMessage ->
    Assert.Equal("@bot", textMessage.BotUsername)
    Assert.Equal("@chat", textMessage.ChatUsername)
    Assert.Equal("user", textMessage.FromUsername)
    Assert.Equal("text", textMessage.MessageText)
    Assert.Equal(message, textMessage.Message)
  | None ->
    Assert.Fail()
...
let ``prepareReplyMessage returns Some when reply is to message``() =
  ...
  | Some replyMessage ->
    Assert.Equal("@bot", replyMessage.BotUsername)
    Assert.Equal("@chat", replyMessage.ChatUsername)
    Assert.Equal("user", replyMessage.FromUsername)
    Assert.Equal("text", replyMessage.MessageText)
    Assert.Equal(replyToUser, replyMessage.ReplyToUser)
    Assert.Equal(message, replyMessage.Message)
  | None ->
    Assert.Fail()
